...
// The Binary Tree solution also works fine with the Binary Search Tree as well
...
public:
    ...
    {
        if (data.size() == 0)
            return nullptr; // If no more elements, return nullptr

        int pos = 0;
        int value = convertStringtoInt(data, pos); // Find new value and position of ','
        if (value < min || value > max)
            return nullptr; // Is new value in given range
